    ...
    public class RestException : Exception, IDisposable
    {
        ...
        public RestException(System.Net.Http.HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                var json = Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonNode.Parse(ResponseBody) as Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonObject;

                // error message could be in properties.statusMessage
                { message = If(json?.Property("properties"), out var p)
                    && If(p?.PropertyT<Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonString>("statusMessage"), out var sm)
                    ? (string)sm : (string)Message; }

                // see if there is an error block in the body
                json = json?.Property("error") ?? json;

                { Code = If(json?.PropertyT<Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonString>("code"), out var c) ? (string)c : (string)StatusCode.ToString(); }
                { message = If(json?.PropertyT<Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonString>("message"), out var m) ? (string)m : (string)Message; }
                { Action = If(json?.PropertyT<Microsoft.Azure.PowerShell.Cmdlets.Authorization.Runtime.Json.JsonString>("action"), out var a) ? (string)a : (string)Action; }
            }
#if DEBUG
